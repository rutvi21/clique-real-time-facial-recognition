...
class DataManager(object):
    ...
    def __init__(self, dataset_name='imdb',
        ...
        if self.dataset_path is not None:
            ...
            self.dataset_path = 'C:/Users/lenovo/PycharmProjects/face_classification/datasets/fer2013/fer2013.csv'
        elif self.dataset_name == 'aug_fer2013':
            self.dataset_path = 'C:/Users/lenovo/PycharmProjects/face_classification/src/output0.csv'
    ...
    def get_data(self):
        if self.dataset_name == 'imdb':
            ...
            ground_truth_data = self._load_aug_fer2013()
        elif self.dataset_name == 'KDEF':
    ...
    def _load_aug_fer2013(self):
        ...
        for emotion in lists:
            files = os.listdir(path + "/" + emotion)
            for file in files:
                path_file = path + "/" + emotion + "/" + file
                file_lst.append(path_file)
                img_file = cv2.imread(path_file, 1)
                width, height = 48, 48
                #format1 = img_file.format
                #mode = img_file.mode
                #img_grey = img_file.convert('L')
                faces = np.asarray(img_file)
                #faces = np.asarray(img_file.getdata(), dtype=np.int).reshape((img_file.size[1], img_file.size[0]))
                #print(faces)
                #print(emotion)
                faces = np.expand_dims(faces, -1)
